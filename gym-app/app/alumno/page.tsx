...
import { RoutineHistory, RoutineExercise, RoutineSet, Exercise } from '@prisma/client';

// Extender el tipo Routine para incluir las relaciones necesarias
type RoutineWithDetails = RoutineHistory & {
...
interface FetchRoutinesResponse {
...
export default function AlumnoPage() {
  ...
  const [routines, setRoutines] = useState<RoutineWithDetails[]>([]); // Actualizar el tipo del estado
  const [hasRoutines, setHasRoutines] = useState<boolean>(false);
  ...
  useEffect(() => {
    const fetchRoutines = async () => {
      try {
        ...
        const data: FetchRoutinesResponse = await res.json();
          ...
          <div className="space-y-8">
            {routines.map((routine, index) => (
              <div key={routine.id} className="mb-8">
                ...
                <RoutineViewer routine={routine} isEditable={routine.status === 'ACTIVE'} />
